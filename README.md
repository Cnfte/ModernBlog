...
- 三级封面图机制：单篇文章 frontmatter `cover` 字段 > 全局固定图池随机抽取 > 随机图 API（带防撞图 uid 参数），逐级兜底，零构建期网络开销。
...
**第四步：填写封面 URL（可选）**
在"封面 URL"输入框中可填写本篇文章专属的封面图直链，对应 front matter 的 `cover` 字段。该字段优先级最高，会覆盖全局"文章封面图池"（`post_bg_urls`）和"随机图 API"（`random_img_api`）的设置。留空则按照三级优先级自动兜底：图池随机抽取 → 随机图 API（带防撞图 uid）→ 全局默认背景图（`bg_url`）。
...
在左侧文件列表中点击任意文章条目，右侧编辑区会自动加载该文章的标题、日期、标签、封面 URL 和正文内容。直接修改后点击"保存"按钮即可覆盖原文件。
...
封面图按三级优先级生效：①单篇文章的"封面 URL"（frontmatter `cover` 字段）；②全局"文章封面图池"（`post_bg_urls`），每篇文章在构建时随机抽取一张；③"随机图 API"（`random_img_api`），构建时不发起网络请求，而是在 API 地址后拼接每篇文章的唯一 `uid` 参数，由浏览器在访问时各自请求，因此正常情况下不会撞图。若某篇文章背景与预期不符，优先检查该文章是否单独填写了"封面 URL"（会覆盖全局设置）；若多篇文章背景重复，且未使用图池，通常是因为这些文章的 `uid` 相同（例如手动批量复制了 `.md` 文件导致 `uid` 未重新生成），正常通过后台新建或保存的文章不会出现此问题。如果希望在构建期就完全确定背景图（而非依赖前端各自请求），改用 `post_bg_urls` 图池。
...
**许可说明**：
此强著佐权许可证的权限取决于必须提供完整源代码（包括使用本许可证作品的较大作品及其修改版本），并在相同许可证下发布。必须保留版权和许可证声明。贡献者明确授予专利权。
