...
```
=======================================================
  CNFTE GEEK_ADMIN // MOE_SYSTEM v8.0.0  (builder.py v3.1)
  Flask WebUI Edition
...
**post_bg_urls**
文章封面图固定图池。在后台"配置"标签中每行填写一个图片直链 URL。构建时，每篇文章会从列表中随机抽取一张，使不同文章在视觉上产生差异。若此列表为空，则回退使用下方的 `random_img_api`；若两者均为空，则回退到 `bg_url`。示例（三行）：
...
单篇文章在编辑时手动填写「封面 URL」（front matter `cover` 字段）可覆盖此全局设置，优先级最高。

**random_img_api**
随机图片 API 地址，仅在 `post_bg_urls` 为空时生效。构建时不会发起任何网络请求，而是将该 API 地址拼接每篇文章独有的 `uid` 参数（例如 `https://www.dmoe.cc/random.php?v=174233xxxxxx`），由浏览器在访问时各自请求，天然防止不同文章出现相同背景图，且零构建开销。推荐填写支持随机返回图片的 API，例如 `https://www.dmoe.cc/random.php`。
...
Token 需在 GitHub → Settings → Developer settings → Personal access tokens 中创建，权限勾选 `repo`（或细粒度权限中的 `Contents: Read and write`）。Token 是敏感凭证，请勿提交到任何公开仓库，此处仅写入本地 `content/config.json`，不会出现在推送记录中。
...
`deploy_repo` 支持 SSH 地址（`git@github.com:…`）和 HTTPS+Token 地址（`https://用户名:token@github.com/…`），部署脚本直接将配置值作为 remote origin 使用，两种格式均可正常推送，无需额外设置。环境变量 `GIT_TERMINAL_PROMPT=0` 已内置，防止命令行等待交互输入挂起。
...
**Q：配置了随机图 API，但所有文章背景都一样**

`random_img_api` 是前端模式：构建时系统会在 API 地址后拼接每篇文章的唯一 uid 参数（例如 `?v=174233xxxxxx`），由浏览器在访问时各自请求。因此如果多篇文章 uid 相同（例如手动批量复制了 `.md` 文件），才会出现背景重复的情况。正常流程下（通过后台新建或保存），每篇文章都会自动生成不同的 uid，不会撞图。如果希望在构建期就确定背景，改用 `post_bg_urls` 图池。
...
*本文档基于 ModernBlogPanel Ver1.0 编写，适用于 main.py v8.0.0 与 builder.py v3.1。*
