...
<div class="tabbar">
  ...
  <div class="tab" data-tab="audio">🎵 音乐</div>
...
<div class="workspace">
  ...
  <div class="pane" id="tab-settings" style="display:none">
    ...
    <div class="card">
      <div class="ck-wrap">
        ...
        <label for="ck-player" style="color:var(--dim);font-size:.8rem">
          前台显示音乐播放器（歌曲来自「🎵 音乐」tab 上传的音频库）
        </label>
      </div>
      <label class="field-label">默认播放顺序</label>
      <select data-cfg="player_default_mode">
    ...
    <div class="card">
      ...
      <label class="field-label" style="margin-top:10px">联系方式（显示在首页头像下方 / 侧栏个人信息卡片，可自由增删排序）</label>
      <div class="btn-row">
        <button class="btn green" onclick="openContactModal()">+ 添加</button>
        <button class="btn" onclick="editContact()">✏ 编辑</button>
        <button class="btn red" onclick="deleteContact()">✕ 删除</button>
        <button class="btn" onclick="moveContact(-1)">↑ 上移</button>
        <button class="btn" onclick="moveContact(1)">↓ 下移</button>
        <button class="btn blue" onclick="saveContactList()">💾 保存联系方式</button>
      </div>
      <table id="contact-table">
        <thead><tr><th>名称</th><th>图标 class</th><th>链接</th></tr></thead>
        <tbody id="contact-tbody"></tbody>
      </table>
      <p style="font-size:.72rem;color:var(--dim);margin-top:6px">
        图标填 Font Awesome 的 class，例如 <code>fab fa-github</code> / <code>fab fa-telegram</code> /
        <code>fas fa-envelope</code>；邮箱请写成 <code>mailto:you@example.com</code>。
      </p>
    </div>
  ...
  <div class="pane" id="tab-audio" style="display:none">
    ...
    <div class="btn-row">
      ...
      <button class="btn" onclick="editAudioTrack()">✏ 编辑信息</button>
      <label class="btn" style="cursor:pointer">
        📝 上传 lrc 歌词 <input type="file" id="lrc-upload" accept=".lrc" style="display:none">
      </label>
      <button class="btn" onclick="removeAudioLrc()">✕ 移除 lrc</button>
      <button class="btn red" onclick="deleteAudioTrack()">🗑 删除</button>
      <button class="btn" onclick="moveAudio(-1)">↑ 上移</button>
      <button class="btn" onclick="moveAudio(1)">↓ 下移</button>
      <button class="btn blue" onclick="saveAudioOrder()">💾 保存顺序</button>
    </div>
    ...
    <p style="font-size:.72rem;color:var(--dim);margin-top:8px">
      点击表格中一行以选中，再进行编辑 / 上传 lrc / 删除 / 排序等操作。歌词来源可在「编辑信息」里选择：
...
<div class="modal-overlay" id="contact-modal">
  <div class="modal">
    ...
    <label class="field-label">名称</label><input id="cm-label" placeholder="例如 GitHub / 邮箱 / QQ">
    <label class="field-label">图标 class（Font Awesome）</label><input id="cm-icon" placeholder="fab fa-github">
    <label class="field-label">链接</label><input id="cm-url" placeholder="https://... 或 mailto:you@example.com">
    <div class="modal-footer">
      <button class="btn" onclick="closeModal('contact-modal')">取消</button>
      <button class="btn green" onclick="confirmContact()">保存</button>
    </div>
  </div>
</div>
...
<div class="modal-overlay" id="audio-modal">
  <div class="modal">
    <h3>编辑音轨信息</h3>
    <input type="hidden" id="am2-id" value="">
    <label class="field-label">标题</label><input id="am2-title">
    <label class="field-label">歌手</label><input id="am2-artist">
    <label class="field-label">歌词来源</label>
    <select id="am2-lyrics-source">
      <option value="none">不显示歌词</option>
      <option value="embedded">使用内嵌歌词</option>
      <option value="lrc">使用上传的 lrc</option>
    </select>
    <p id="am2-lyrics-hint" style="font-size:.7rem;color:var(--dim);margin-top:4px"></p>
    <div class="modal-footer">
      <button class="btn" onclick="closeModal('audio-modal')">取消</button>
      <button class="btn green" onclick="confirmAudioTrack()">保存</button>
    </div>
  </div>
</div>
...
function switchTab(tabName) {
  ...
  if (tabName === 'settings') { loadSettings(); loadContacts(); }
  if (tabName === 'anime') loadAnime();
  if (tabName === 'friends') loadFriends();
  if (tabName === 'audio') loadAudio();
...
async function loadAssets() {
  ...
  files.forEach(f => {
    ...
    sizeDiv.className = 'ai-size';
    sizeDiv.textContent = f.size;
    const linkDiv = document.createElement('div');
    linkDiv.className = 'asset-link';
    linkDiv.textContent = link;
    linkDiv.onclick = () => selectAsset(link, linkDiv);
    d.appendChild(nameDiv);
    d.appendChild(sizeDiv);
...
async function openCoverPicker() {
  ...
  imgFiles.forEach(f => {
    ...
    const nameDiv = document.createElement('div');
    nameDiv.className = 'ai-name';
    nameDiv.style.fontSize = '0.72rem';
    nameDiv.textContent = f.name;
    d.appendChild(img);
    d.appendChild(nameDiv);
    d.onclick = () => {
...
function renderAnimeTable() {
  ...
  animeList.forEach((a, i) => {
    ...
    tr.innerHTML = `<td>${escapeHtml(a.title||'')}</td><td>${a.status==='airing'?'连载中':'完结'}</td>
      <td>${escapeHtml(a.ep||'')}</td><td>${escapeHtml(a.total||'')}</td>
      <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis">${escapeHtml(a.cover||'')}</td>
...
function renderFriendTable() {
  ...
  friendList.forEach((f, i) => {
    ...
    tr.innerHTML = `<td>${escapeHtml(f.name||'')}</td>
      <td style="max-width:200px;overflow:hidden;text-overflow:ellipsis">${escapeHtml(f.url||'')}</td>
      <td style="max-width:160px;overflow:hidden;text-overflow:ellipsis">${escapeHtml(f.avatar||'')}</td>
...
// ─── Contact Methods (联系方式，显示在首页头像下方 / 侧栏个人信息卡片) ───
...
function renderContactTable() {
  ...
  if (!contactList.length) {
    tb.innerHTML = '<tr><td colspan="3" style="color:var(--dim)">还没有联系方式，点击「+ 添加」新增</td></tr>';
    return;
  }
  contactList.forEach((c, i) => {
    const tr = document.createElement('tr');
    if (i === selectedContactRow) tr.classList.add('selected');
    tr.innerHTML = `<td>${escapeHtml(c.label||'')}</td><td><i class="${escapeHtml(c.icon||'')}"></i> ${escapeHtml(c.icon||'')}</td><td style="word-break:break-all">${escapeHtml(c.url||'')}</td>`;
    tr.onclick = () => { selectedContactRow = i; renderContactTable(); };
    tb.appendChild(tr);
  });
}

function openContactModal(data, idx) {
...
async function saveContactList() {
  ...
  setStatus('✅ 联系方式已保存'); alert('联系方式已保存！重新构建后生效。');
}
...
function renderAudioTable() {
  ...
  tb.innerHTML = '';
  if (!audioList.length) {
    tb.innerHTML = '<tr><td colspan="5" style="color:var(--dim)">音频库为空，点击「📁 上传音频」添加</td></tr>';
    return;
  }
  audioList.forEach((a, i) => {
    const tr = document.createElement('tr');
    if (i === selectedAudioRow) tr.classList.add('selected');
    tr.innerHTML = `<td>${escapeHtml(a.title||'')}</td><td>${escapeHtml(a.artist||'')}</td>
      <td>${escapeHtml((a.ext||'').replace('.','').toUpperCase())}</td><td>${escapeHtml(a.duration_h||'')}</td>
      <td>${LYRICS_SOURCE_LABEL[a.lyrics_source]||'无'}</td>`;
...
document.getElementById('audio-upload').onchange = async (e) => {
  ...
  e.target.value = '';
  if (d.ok) {
    setStatus(`✅ 成功上传 ${d.added.length} 首` + (d.errors.length ? `，${d.errors.length} 个失败` : ''));
    if (d.errors.length) alert('部分文件上传失败:\\n' + d.errors.join('\\n'));
    loadAudio();
  } else {
    alert('上传失败: ' + d.error); setStatus('上传失败', false);
  }
};

function editAudioTrack() {
  if (selectedAudioRow < 0) { alert('请先选中一行'); return; }
  ...
  document.getElementById('am2-lyrics-source').value = a.lyrics_source || 'none';
  const hints = [];
  hints.push(a.has_embedded_lyrics ? '✅ 检测到内嵌歌词' : '✕ 无内嵌歌词');
  hints.push(a.has_lrc ? '✅ 已上传 lrc' : '✕ 未上传 lrc');
...
async function confirmAudioTrack() {
  ...
  const body = {
    id,
    title: document.getElementById('am2-title').value.trim(),
    artist: document.getElementById('am2-artist').value.trim(),
    lyrics_source: document.getElementById('am2-lyrics-source').value,
  };
  const res = await fetch('/api/audio/update', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(body)});
  const d = await res.json();
  closeModal('audio-modal');
  if (d.ok) { setStatus('✅ 音轨信息已更新'); loadAudio(); }
  else alert('保存失败: ' + d.error);
}
...
function deleteAudioTrack() {
  if (selectedAudioRow < 0) { alert('请先选中一行'); return; }
  const a = audioList[selectedAudioRow];
  if (!confirm(`删除音轨「${a.title}」？此操作不可撤销。`)) return;
  fetch('/api/audio?id=' + encodeURIComponent(a.id), {method:'DELETE'})
    .then(() => { setStatus('✅ 音轨已删除'); loadAudio(); });
}
...
async function saveAudioOrder() {
  ...
  await fetch('/api/audio/reorder', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(ids)});
  setStatus('✅ 播放顺序已保存');
}
...
document.getElementById('lrc-upload').onchange = async (e) => {
  ...
  if (!f) return;
  if (selectedAudioRow < 0) { alert('请先在表格中选中一首歌'); e.target.value=''; return; }
  const a = audioList[selectedAudioRow];
  const fd = new FormData();
  fd.append('id', a.id);
  fd.append('file', f);
  const res = await fetch('/api/audio/lrc', {method:'POST', body:fd});
  const d = await res.json();
  e.target.value = '';
  if (d.ok) { setStatus('✅ lrc 歌词已上传'); loadAudio(); }
...
async function removeAudioLrc() {
  if (selectedAudioRow < 0) { alert('请先选中一行'); return; }
  const a = audioList[selectedAudioRow];
  if (!confirm(`移除「${a.title}」的 lrc 歌词？`)) return;
  const res = await fetch('/api/audio/lrc?id=' + encodeURIComponent(a.id), {method:'DELETE'});
  const d = await res.json();
  if (d.ok) { setStatus('✅ lrc 已移除'); loadAudio(); }
...
async function loadBackups() {
  ...
  list.forEach(b => {
    ...
    if (b.is_snapshot) {
      ...
      tagSpan.style.color = 'var(--accent2)';
      tagSpan.textContent = ' [恢复前快照]';
      nameTd.appendChild(tagSpan);
    }
...
async function loadCloudBackups() {
  ...
  d.backups.forEach(b => {
    ...
    btnPull.style.cssText = 'padding:4px 10px';
...
def api_upload():
    ...
    for f in files:
        ...
        if ext not in ALLOWED_EXTENSIONS:
            _broadcast_log(f"⚠️ 拒绝上传（不安全的类型）: {fn}")
            continue
        # 文件名只保留安全字符
...
def api_contact_methods():
    ...
    cfg = _load_config()
    cfg['contact_methods'] = request.json
    _save_config(cfg)
...
def api_audio_upload():
    ...
    for f in files:
        try:
            ...
            added.append(track)
            _broadcast_log(f"🎵 音频上传: {f.filename} → {track['title']}")
        except Exception as e:
...
def api_audio_update():
    ...
    try:
        ...
        _broadcast_log(f"🎵 音轨信息已更新: {track['title']}")
...
def api_audio_lrc_upload():
    ...
    try:
        ...
        _broadcast_log(f"🎵 已为《{track['title']}》上传 lrc 歌词")
        return jsonify({'ok': True, 'track': track})
    except FileNotFoundError as e:
        return jsonify({'ok': False, 'error': str(e)})
